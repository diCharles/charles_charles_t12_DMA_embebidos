...
uint8_t g_data_desti[4*ARRAY_SIZE]; //defines destination data space
...
void DMA_init(void)
{
	...
	uint8_t word_width_in_bytes = 1;
	uint32_t 	words_to_trasmmit =16;
	DMAMUX->CHCFG[0] = 0;
	...
	DMA0->TCD[0].SOFF = 1*word_width_in_bytes;/*Source address signed offset;it is expressed in number of bytes*/
	DMA0->TCD[0].DADDR = (uint32_t)(&g_data_desti[0]);/*defines destination data address*/
	DMA0->TCD[0].DOFF = 1*word_width_in_bytes;/*destination address signed offset;it is expressed in number of bytes*/
	...
	DMA0->TCD[0].NBYTES_MLNO = (words_to_trasmmit*word_width_in_bytes )/NUM_STEPS;/*byte number for transference*/

	DMA0->TCD[0].ATTR = DMA_ATTR_SSIZE(word_width_in_bytes-1);/*8 bit transfer size, in order to transfer see Kinetis user manual*/
	...
	DMA0->TCD[0].DLAST_SGA = -words_to_trasmmit*word_width_in_bytes;/*restores the destination address to the initial value, which is expressed in the amount of bytes to restore*/
