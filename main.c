...
void DMA_init(void)
{
   /** PARAMETERS TO MODIFY*/
	...
	uint32_t 	words_to_trasmmit =  3  * word_width_in_bytes;
	uint32_t citer_steps = 1;

	uint32_t source_offset = 2; /** write this on words, a word not from k64 archq , a word from DMA transfer*/
	...
	DMA0->TCD[0].NBYTES_MLNO = (words_to_trasmmit*word_width_in_bytes ) / 2 ;/*byte number for transference*/

	DMA0->TCD[0].ATTR = DMA_ATTR_SSIZE(word_width_in_bytes-1);/*16 bit transfer size, in order to transfer see Kinetis user manual*/
	DMA0->TCD[0].SLAST = -10*word_width_in_bytes;//restores the source address to the initial value, which is expressed in the amount of bytes to restore*/
